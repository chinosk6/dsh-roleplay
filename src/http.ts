...
async function route(req: IncomingMessage, res: ServerResponse, service: RoleplayService): Promise<void> {
  ...
  if (path === '/cards' && method === 'GET') {
    sendJson(res, 200, {
      cards: service.listCards().map(card => ({
        id: card.id,
        name: card.name,
        ...
        avatarUrl: service.avatarUrl(card) ?? null,
  ...
  if (path === '/cards' && method === 'POST') {
    const card = await service.saveCard(JSON.parse((await readBody(req)).toString('utf8')))
    sendJson(res, 200, { card })
    return
  }
  if (path === '/cards/import' && method === 'POST') {
    const card = await service.importCard(await readBody(req))
    sendJson(res, 200, { card, avatarUrl: service.avatarUrl(card) ?? null })
    return
  }
  ...
  if (cardMatch) {
    ...
    if (rest === '' && method === 'PUT') {
      ...
      const card = await service.saveCard({ ...service.getCard(id), ...body, id })
      sendJson(res, 200, { card })
      return
    }
    if (rest === '' && method === 'DELETE') {
      sendJson(res, 200, { deleted: await service.deleteCard(id) })
      return
    }
    if (rest === '/export.json' && method === 'GET') {
      ...
      res.end(service.exportCardJson(id))
      return
    }
    if (rest === '/export.png' && method === 'GET') {
      const card = service.getCard(id)
      if (!card) return sendJson(res, 404, { error: 'card not found' })
      const png = await service.exportCardPng(id)
      res.writeHead(200, {
    ...
    if (rest === '/avatar' && method === 'PUT') {
      const card = await service.setAvatar(id, await readBody(req))
      sendJson(res, 200, { card, avatarUrl: service.avatarUrl(card) ?? null })
      return
    }
    if (rest === '/avatar' && method === 'DELETE') {
      const card = await service.deleteAvatar(id)
      sendJson(res, 200, { card, avatarUrl: service.avatarUrl(card) ?? null })
      return
    }
  }
  ...
  if (sessionMatch) {
    ...
    if (!isReference && method === 'GET') {
      ...
      const card = binding?.characterId ? service.getCard(binding.characterId) : undefined
      sendJson(res, 200, {
  ...
  if (path === '/images' && method === 'GET') {
    sendJson(res, 200, await service.listImages())
    return
  }
  if (path === '/images/state' && method === 'GET') {
    const ids = (url.searchParams.get('ids') ?? '').split(',').filter(Boolean)
    sendJson(res, 200, { states: service.imageStates(ids) })
    return
  }
  if (path === '/images/star' && method === 'POST') {
    ...
    await service.setImageStarred(body.id, body.starred === true)
    sendJson(res, 200, { ok: true })
    return
  }
  if (path === '/images/delete' && method === 'POST') {
    ...
    sendJson(res, 200, { deleted: await service.deleteImages(ids) })
    return
  }
  if (path === '/images/regenerate' && method === 'POST') {
    const body = JSON.parse((await readBody(req)).toString('utf8')) as { ids?: string[] }
    const ids = Array.isArray(body.ids) ? body.ids.filter(id => typeof id === 'string') : []
    sendJson(res, 200, { regenerated: await service.regenerateImages(ids) })
    return
  }
  ...
  if (fileMatch && method === 'GET') {
    const bytes = await service.readDataFile(fileMatch[1] as 'avatars' | 'images', fileMatch[2]!)
